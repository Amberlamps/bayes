...
Naivebayes.prototype.learn = function (text, category) {
  ...
  .forEach(function (token) {
    ...
    if (!self.wordFrequencyCount[category].hasOwnProperty(token))
...
Naivebayes.prototype.categorize = function (text) {
  ...
  .forEach(function (category) {
    ...
    categories[categories.length] = {
      cat: category,
      prob: categoryProbability
    };

    total += categoryProbability;
  ...
  return {
    ...
    distribution: categories.map(function (cat) {
      return {
        cat: cat.cat,
        prob: cat.prob / total
      }
...
Naivebayes.prototype.frequencyTable = function (tokens) {
  ...
  tokens.forEach(function (token) {
    if (!frequencyTable.hasOwnProperty(token))
